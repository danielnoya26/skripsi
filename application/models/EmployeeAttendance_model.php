	...
	public function insertAttendace($input, $dataToken)
	{
		...
		if ($dataConfAttendance) {
			...
			if (in_array($today,$configDays) == false) {
				...
				$imageDataUrl = $input->post('imageDataURL');
					$base64Image = explode(',', $imageDataUrl)[1];

					$imageData = base64_decode($base64Image);
					$employeeId = $employeeData->employeeId;

					$uploadPath = "assets/img/attendance_photo/$employeeId";
				...
				if (isset($todayAttend->time_arrives) && $todayAttend->time_arrives && isset($todayAttend->time_departure) && $todayAttend->time_departure) {
					...
					if ($result > $dataConfAttendance->max_distance_attendance) {
						...
						if (is_null($todayAttend)) {
							...
							$name = $employeeId."_arrives_".date("Ymd", strtotime("today"));

							$uploadPath = $uploadPath."/".$name.".png";
							if (file_put_contents($uploadPath, $imageData)) {
								$upload_filename =$name.".png";
								$upload_filepath = $uploadPath;
							}else{
								$upload_filename="";
								$upload_filepath="";
							}
							...
							$this->db->set('upload_filename_arrives', $upload_filename);
							$this->db->set('upload_filepath_arrives', $upload_filepath);
							$this->db->insert('employee_attendances');
						}

						elseif ($todayAttend->time_arrives && is_null($todayAttend->time_departure)) {
							$name = $employeeId."_departure_".date("Ymd", strtotime("today"));
							...
							$this->db->set('upload_filename_departure', $upload_filename);
							$this->db->set('upload_filepath_departure', $upload_filepath);
